...
# ╔═╡ 1b4a84a4-2999-4a1c-ad1f-4443588f257f
md"""
---
## Initial Condition
...
# ╔═╡ 1b0f5d58-cd28-4edd-b1d8-2be292ee682d
@bind values PlutoUI.combine() do Child
	md"""
	h $(
		Child(Slider(1.5:0.1:4))
	) 
	
	E $(
		Child(Slider(-200:0.1:5))
	) 
	"""
end

# ╔═╡ f919e297-1383-47ac-81b8-236c530a5485
begin
	...
	h,E = values
	r = LinRange(0.05,5.,500) 
	
	hline([0], color="black", alpha=0.5, label=false)
	plot!(r, eff_potential.(r,h), color="crimson", label=L"$V_{eff} (r)$"
	     ,title="Effective Potential  h="*string(h), titlefontsize=10
	     , xlabel=L"$r$", ylabel=L"$V_{eff} (r)$, $E$"
		 , ylim=(-200,10))
	
	hline!([E],color="cornflowerblue", label="E = "*string(E))
end
...
begin
	...
	# Plot
	plot(QE[:,2], QE[:,3], color="crimson", label=L"$dt =$ "*string(QE[1,1]- QE[2,1])
	     , xlabel=L"$x$" , ylabel=L"$y$"
	     , title="Forward Euler's Method", titlefontsize=10)
end

# ╔═╡ 3020b3e6-cee1-4a82-b256-1f418a9bd832
md"""
---
...
# ╔═╡ f18e243f-27e8-4dad-aa59-20e150ac346d
...
# ╔═╡ d1402a11-81de-4073-88b2-ebccfb034bf4
begin
	QEII = FEuler(f, t0, Q0, tf, 10*n)
	
	energy, angMom = conservedQ(QE, n)
	energyII, angMomII = conservedQ(QEII, n)

	dt = QE[1,1]- QE[2,1]
	dtII = QEII[1,1]- QEII[2,1]
...
# ╔═╡ 99afff56-ea7c-4ee9-a95e-16e95f625368
md"""
These plots show clearly that the forward Euler's method with a not-so-small setpsize does not satisfy the conservation of energy (and produces a small increase in the angular momentum).
"""

# ╔═╡ fd7f560b-33fd-4643-9c58-1399ac25a1ae
println("El cambio en la energía utilizando un paso de dt = "*string(dt)* " es: ", round(abs(energy[end] - energy[1]),digits=4))

# ╔═╡ 4835929f-4298-49ba-96eb-6bfab3f4aa7a
println("El cambio en la energía utilizando un paso de dt = "* string(dtII)*" es : ", round(abs(energyII[end] - energyII[1]),digits=4))
