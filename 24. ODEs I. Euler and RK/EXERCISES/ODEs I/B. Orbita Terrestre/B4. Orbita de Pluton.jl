...
md"""
# B4. Orbita de Plutón. Integradores Simplécticos.
...
# ╔═╡ 356ae254-ec20-40ab-8c8e-bba25ec55f66
md"""
...
La masa de la Plutón, $m$, es  mucho más pequeña que la masa del Sol, i.e. $m \ll M$. Este hecho hace que el Sol permanezca estático mientras el cometa se mueve en orbita alrededor suyo bajo la influencia gravitacional.

Las ecuaciones dinámicas del planeta se pueden escribir como el siguiente sistema de 4 ecuaciones de primer orden restringidas al plano del movimiento,
...
Tomaremos $t=0$ en el perihelio. Allí la distancia entre los dos cuerpos es de $4.4368 \times 10^{12} \text{ m}$ y su velocidad lineal es de $6.1218 \times 10^3 \text{ m/s}$. De esta forma se tomará la siguiente condición incial:
...
function velVerlet(ODE, t0, q0, tf, dt)
    """
    ------------------------------------------
    ...
    ------------------------------------------
    """
	d = trunc(Int,length(q0)/2) # Dimension of the system 
    n = trunc(Int,(tf - t0)/dt) # Number of steps
	...
	for i in 2:n
        q[i,1] = q[i-1,1] + dt
        v_half = q[i-1,d+2:end] .+ ODE(q[i-1,1], q[i-1,2:end])[d+1:end]*(dt/2)
        q[i,2:d+1] = q[i-1,2:d+1] .+ v_half*dt
        q[i,d+2:end] = v_half .+ ODE(q[i,1], q[i,2:end])[d+1:end]*(dt/2)
	end
    return q
end
...
begin 
	q1, cq1= trajectory()
	# Plot Trajectory
	plot(q1[:,2], q1[:,3], color="crimson", label=L"$dt = $"*string(q1[2,1]-q1[1,1])
	 , xlabel=L"$x$" , ylabel=L"$y$"
	 , title="Orbit with the Velocity-Verlet Algorithm", titlefontsize=10)
end

# ╔═╡ 56a613d3-ec96-49ec-b9ed-03e24ac456ed
# Plot Trajectory
plot(q1[:,1], q1[:,2], color="cornflowerblue", label=L"$dt = $"*string(q1[2,1]-q1[1,1])
...
function plotTrajectory(q,cq)
	p1 = plot(q[:,2], q[:,3], color="crimson", label=L"$dt = $"*string(q1[2,1]-q1[1,1])
		 , xlabel=L"$x$" , ylabel=L"$y$"
...
println("El cambio en la energía utilizando un algoritmo velocity-Verlet es: ", round(abs(cq1[end,2] - cq1[1,2]),digits=20))
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "f71089c0bda3bf80f1ec1db4b3af2622fd91baa3"
