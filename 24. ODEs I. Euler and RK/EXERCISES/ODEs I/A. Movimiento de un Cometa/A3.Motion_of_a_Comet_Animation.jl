...
md"""
# A1. Movimiento de un Cometa. Método RK4.
...
function RK4(ODE, t0, q0, tf, n)
    ...
    for i in 2:n
        q[i,1] = q[i-1,1] + dt
        k1 = dt.*ODE(q[i-1,1], q[i-1,2:end])
        k2 = dt.*ODE(q[i-1,1] .+ dt./2, q[i-1,2:end] .+ k1./2)
        k3 = dt.*ODE(q[i-1,1] .+ dt./2, q[i-1,2:end] .+ k2./2)
        k4 = dt.*ODE(q[i-1,1] .+ dt, q[i-1,2:end] .+ k3)
        q[i,2:end] = q[i-1,2:end] .+ (k1 .+ 2*k2 .+ 2 .*k3 .+ k4)./6
    end
    return q
end
...
begin 
	...
	# Plot Trajectory
	plot(q1[:,2], q1[:,3], color="crimson", label=L"$dt =$ "*string(q1[2,1]- q1[1,1])*" yr"
	 , xlabel=L"$x$" , ylabel=L"$y$"
	 , title="Orbit with the RK4 Method", titlefontsize=10)
end

# ╔═╡ 56a613d3-ec96-49ec-b9ed-03e24ac456ed
# Plot Trajectory
plot(q1[:,1], q1[:,2], color="cornflowerblue", label=L"$dt =$ "*string(q1[1,1]- q1[2,1])*" yr"
...
function plotTrajectory(q,cq)
	p1 = plot(q[:,2], q[:,3], color="crimson", label=L"$dt =$ "*string(q[1,1]- q[2,1])*" yr"
		 , xlabel=L"$x$" , ylabel=L"$y$"
...
println("El cambio en la energía utilizando un paso de dt = "*string(q1[2,1]- q1[1,1])*" es: ", round(abs(cq1[end,2] - cq1[1,2]),digits=4))

# ╔═╡ 17169689-7c5a-4832-8e59-19eed0f5f72e
begin 
	q2, cq2= trajectory(2000000)
	# Plot Trajectory
	plot(q2[:,2], q2[:,3], color="crimson", label=L"$dt =$ "*string(q2[2,1]- q2[1,1])*" yr"
	 , xlabel=L"$x$" , ylabel=L"$y$"
	 , title="Orbit with the RK4 Method", titlefontsize=10)
end
...
# Plot Trajectory
plot(q2[:,1], q2[:,2], color="cornflowerblue", label=L"$dt =$ "*string(q2[1,1]- q2[2,1])*" yr"
	 , xlabel=L"$t$" , ylabel=L"$x$"
	 , size=(900,400)
	 , title=L"$x$ coordinate as function of time", titlefontsize=10)

# ╔═╡ 5b536387-e1d0-4a25-98b9-cee8739c1b0e
plotTrajectory(q2,cq2)

# ╔═╡ 707b5e55-24d9-42f8-acfc-3f17f986611a
println("El cambio en la energía utilizando un paso de dt = "*string(q2[2,1]- q2[1,1])*" es: ", round(abs(cq2[end,2] - cq2[1,2]),digits=7))

# ╔═╡ 1cc1e03e-d181-47c3-b610-8cd9fc2927dd
begin

xx = q2[1:2000:330000,2]
yy = q2[1:2000:330000,3]
n = length(xx)

anim = @animate for i ∈ 1:n
	plot(q2[1:50:330000,2],q2[1:50:330000,3], label=false)
    scatter!(xx[i:i], yy[i:i]
	, xlim=(-1,30), ylim=(-2,2), label=false)
end
