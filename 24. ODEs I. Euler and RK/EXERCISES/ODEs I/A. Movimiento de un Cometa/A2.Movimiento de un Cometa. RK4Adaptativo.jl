...
# ╔═╡ 09dcaf7e-4b88-4327-a017-197635173f71
begin
	using Plots
	using LaTeXStrings
	using PlutoUI
...
# ╔═╡ a5a94bb0-b36a-4388-90f8-7607b8610a65
md"""
# A1. Movimiento de un Cometa. Método RK4 con paso adaptativo.

Eduard Larrañaga (ealarranaga@unal.edu.co)
---
"""

# ╔═╡ e1612c53-c68b-4a44-b92a-6aacfa123530
md"""
...
donde $r = \sqrt{x^2 + y^2}$, $G$ es la constante gravitacional de Newton y $M$ es la masa del Sol.
	...
	1. Ahora escriba un programa que resuelva el problema utilizando uno de los métodos RK con paso adaptativo. Establezca la tolerancia de tal forma que se tenga una precisión de $1 \text{ km}$ por año en la posición del cometa. Como se comparan los resultadosde los dos métodos?
    Realice un gráfico de $x$ vs. $t$ y otro de $y$ vs. $t$ localizando los puntos correspondientes a los calculos con paso adaptativo para confirmar que el tamaño cambia de acuerdo a la ubicación del cometa con respecto al Sol. Como condición inicial, suponga que en $t=0$ el cometa se encuentra en las coordenadas
...
# ╔═╡ 85b8c021-a1e7-48b8-9827-6995cf456d1a
md"""
---
**Solución**
...
# ╔═╡ c415b111-6707-474f-8336-47e287f40688
begin
	G = 4. *pi^2 # Newtonian Gravitational Constant
	M = 1.
end

# ╔═╡ f0dc517a-6b6a-11ee-2ffd-fdeb3dbde18e
function f(t0, q0)
    """
    ------------------------------------------
    f(t0, q0) 
...
# ╔═╡ 0d880825-28fe-4f1f-81b5-54b9b4c877e8
function conserv_quant(q)
    """
    ------------------------------------------
    conserv_quant(q)
    ------------------------------------------
...
# ╔═╡ 0a45d58a-9ba1-48dd-81fb-c9f5e6057d58
md"""
---
**Condición Inicial**
...
# ╔═╡ e1417eba-28f5-4879-ba58-9173a861ba59
# Initial Conditions
begin
	x = 4e9 # [km]
	y = 0 # [km]
...
# ╔═╡ cc6ce3fa-b86d-4f32-9116-7514ab6e808d
md"""
...
# ╔═╡ ebe43a8f-8229-4dcd-bdf4-ad3db77ab142


# ╔═╡ bc409fa2-c354-4f89-9412-ed81eb664511
# Adaptative Runge-Kutta 4 Algorithm
function ARK4(ODE, t0, q0, tf, n; dt = 1e-2, epsilon = 1e-8)
    """
    ----------------------------------------------------
    ARK4(ODE, t0, q0, tf, n; dt = 1e-2, epsilon = 1e-8)
    4th Order Runge-Kutta method with adaptative step
	...
	dt: Initial value of the integration stepsize
	epsilon: Tolerance of the adaptive method
    ----------------------------------------------------
    """
	...
	function ks(qn,dt)
		k1 = dt.*ODE(qn[1], qn[2:end])
        k2 = dt.*ODE(qn[1] .+ dt./2, qn[2:end] .+ k1./2)
        k3 = dt.*ODE(qn[1] .+ dt./2, qn[2:end] .+ k2./2)
        k4 = dt.*ODE(qn[1] .+ dt, qn[2:end] .+ k3)
    ...
    while i<n && time<tf 
		# 2 steps with dt
        q[i,2:end] = q[i-1,2:end] .+ ks(q[i-1,:],dt)
        q[i+1,2:end] = q[i,2:end] .+ ks(q[i,:],dt)
        q1 = q[i+1,2:end]
		# 1 step with 2dt
        q2 = q[i-1,2:end] .+ ks(q[i-1,:],2*dt)

		Theta = sqrt((q1[1]-q2[1]).^2 + (q1[2]-q2[2]).^2)/(30*dt*epsilon)
        if Theta<1
            q[i,1] = q[i-1,1] + dt
            q[i+1,1] = q[i,1] + dt
            time += 2*dt
	...
	Q = q[1:i-1,:] # Keep only the obtained data (discard extra zeros in the array)
    println("Se concluye al llegar al tiempo t_f = "*string(time)*", completando un total de "*string(i)*" datos.\n")
    return Q
end

# ╔═╡ ec60c431-ba0c-49f3-9744-3174a06c9687
function trajectory(n)
	...
	return Q, conservedQuantities
end

# ╔═╡ a10603a6-5718-42d3-a318-061fdc635dfc
begin 
	q1, cq1= trajectory(500000)
	# Plot Trajectory
	plot(q1[:,2], q1[:,3], color="crimson", label=false
	 , xlabel=L"$x$" , ylabel=L"$y$"
	 , title="Orbit with the Adaptative RK4 Method", titlefontsize=10)
end

# ╔═╡ 56a613d3-ec96-49ec-b9ed-03e24ac456ed
# Plot Trajectory
plot(q1[:,1], q1[:,2], color="cornflowerblue", label=false
	 , xlabel=L"$t$" , ylabel=L"$x$"
	 , size=(900,400)
	 , title=L"$x$ coordinate as function of time", titlefontsize=10)

# ╔═╡ 9aaaefae-bd23-4f18-a056-5c1facf5a4f2
function plotTrajectory(q,cq)
	p1 = plot(q[:,2], q[:,3], color="crimson", label=false
		 , xlabel=L"$x$" , ylabel=L"$y$"
		 , title="Orbit", titlefontsize=10
	     , left_margin=5*Plots.mm)
	
	p2 = plot(cq[:,1], cq[:,2]
		      , color="darkviolet", label="Energy"
		      , xlabel=L"$t$" , ylabel=L"$E$, $\ell$"
		      , title="Conserved Quantities", titlefontsize=10
	          )
	
	p2 = plot!(p2, cq[:,1], cq[:,3], label="Angular Momentum")

	plot(p1,p2, layout=(1,2), size=(1000,400)
		 , bottom_margin = 5*Plots.mm)
end

# ╔═╡ fe6e7ee5-d632-4b4b-bc3e-3381079d4e64
plotTrajectory(q1,cq1)

# ╔═╡ fd7f560b-33fd-4643-9c58-1399ac25a1ae
println("El cambio en la energía utilizando un paso adaptativo es: ", round(abs(cq1[end,2] - cq1[1,2]),digits=7))

# ╔═╡ ef112c4b-44b4-429c-a39d-db69f42856c4
# Plot Trajectory
begin
plot(q1[1:2000,1], color="black", label=false
	 , seriestype=vline)
plot!(q1[1:2000,1], q1[1:2000,2], color="cornflowerblue", label=false
	 , xlabel=L"$t$" , ylabel=L"$x$"
	 , size=(900,400)
	 , title="Time steps", titlefontsize=10)

end

# ╔═╡ 555c0e0b-8661-44bf-af9d-7143efe6840b
# Plot Trajectory
begin
plot(q1[1:2000,1], color="black", label=false
	 , seriestype=vline)
plot!(q1[1:2000,1], q1[1:2000,4], color="cornflowerblue", label=false
	 , xlabel=L"$t$" , ylabel=L"$x$"
	 , size=(900,400)
	 , title="Time steps", titlefontsize=10)
end
